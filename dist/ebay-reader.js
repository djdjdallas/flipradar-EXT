(()=>{(function(){"use strict";let r="flipchecker-ebay-badge";function u(){let e=window.location.href;return e.includes("LH_Sold=1")||e.includes("LH_Complete=1")}function h(){return new URLSearchParams(window.location.search).get("_nkw")||""}function b(e){if(!e)return null;let t=e.match(/\$[\d,]+\.?\d*/);if(!t)return null;let i=t[0].replace(/[^0-9.]/g,""),o=parseFloat(i);return isNaN(o)?null:o}function m(){let e=[];return document.querySelectorAll(".s-item").forEach((i,o)=>{if(o===0)return;let l=i.querySelector(".s-item__title"),c=i.querySelector(".s-item__price"),a=i.querySelector(".s-item__endedDate")||i.querySelector(".s-item__ended-date")||i.querySelector(".POSITIVE"),f=i.querySelector(".s-item__link");if(l&&c){let s=b(c.textContent);s&&s>0&&s<1e5&&e.push({title:l.textContent.trim().substring(0,100),price:s,soldDate:a?a.textContent.trim():null,url:f?f.href:null})}}),e}function v(e){if(e.length===0)return{count:0,low:null,high:null,avg:null,median:null};let t=e.map(c=>c.price).sort((c,a)=>c-a),i=t.reduce((c,a)=>c+a,0),o=Math.floor(t.length/2),l=t.length%2!==0?t[o]:(t[o-1]+t[o])/2;return{count:t.length,low:t[0],high:t[t.length-1],avg:Math.round(i/t.length),median:Math.round(l),prices:t.slice(0,20)}}function k(e,t,i){let o={query:e.toLowerCase().trim(),stats:t,samples:i.slice(0,10),timestamp:Date.now(),url:window.location.href},l=`flipchecker_sold_${e.toLowerCase().replace(/\s+/g,"_").substring(0,50)}`;chrome.storage.local.set({[l]:o,flipchecker_last_sold:o},()=>{}),chrome.runtime.sendMessage({type:"soldDataCaptured",data:o})}function n(e){return e?`$${e.toLocaleString()}`:"N/A"}function x(e,t){let i=document.getElementById(r);i&&i.remove();let o=document.createElement("div");o.id=r,o.innerHTML=`
      <style>
        #${r} {
          position: fixed;
          top: 80px;
    ...
    `,document.body.appendChild(o),document.getElementById("flipchecker-close-badge").addEventListener("click",()=>{o.remove()}),setTimeout(()=>{document.getElementById(r)&&(o.style.transition="opacity 0.5s",o.style.opacity="0",setTimeout(()=>o.remove(),500))},3e4)}function d(){u()&&setTimeout(()=>{let e=h();if(!e)return;let t=m();if(t.length===0)return;let i=v(t);k(e,i,t),x(i,e)},1500)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",d):d();let g=window.location.href,p=null,_=100;new MutationObserver(()=>{p&&clearTimeout(p),p=setTimeout(()=>{window.location.href!==g&&(g=window.location.href,setTimeout(d,1500))},_)}).observe(document.body,{childList:!0,subtree:!0})})();})();
